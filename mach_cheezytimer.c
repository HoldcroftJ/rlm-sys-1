...
int32_t term(int op, int port, int data) {
    ...
    case TERMSVC:
        ...
        if( term_cheezy_tx_time[0] < m68k_get_cycles_counter() ) {
            BYTE_AT(GIO,T0+Tn_CTLb) |= Tn_CTLb_TXE;
            if( BYTE_AT(GIO,T0+Tn_CTLb) & Tn_CTLb_IRQEN ) {
                BYTE_AT(GIO,T0+Tn_CTLb) |= (Tn_CTLb_TEI);
                m68k_raise_irq(TERMn_IRQLVL, M68K_AUTOVECTORED_IRQ);
            }
        }
        if( term_cheezy_tx_time[1] < m68k_get_cycles_counter() ) {
            BYTE_AT(GIO,T1+Tn_CTLb) |= Tn_CTLb_TXE;
            if( BYTE_AT(GIO,T1+Tn_CTLb) & Tn_CTLb_IRQEN ) {
                BYTE_AT(GIO,T1+Tn_CTLb) |= (Tn_CTLb_TEI);
                m68k_raise_irq(TERMn_IRQLVL, M68K_AUTOVECTORED_IRQ);
            }
        }
        break;
