...
int32_t term(int op, int port, int data) {
    ...
    case TERMINIT:
        {
            if( !context ) { context = zmq_init(1); }
            responder[port] = zmq_socket(context, ZMQ_REP);
            if( !zmq_bind(responder[port], port ? "udp://*:8800" : "udp://*:8801") ) {
                _ERR; perror("zmq_bind"); N_;
            }
        }
        break;
    case TERMDEINIT:
        zmq_close(responder[port]);
        break;
    case TERMREAD:
        {
            zmq_msg_t request;
            ...
            zmq_msg_recv(responder, &request, 0);
            inData = request.data[0];
    ...
    case TERMWRITE:
        {
            ...
            memcpy(zmq_msg_data(&reply), &outData, 1);
