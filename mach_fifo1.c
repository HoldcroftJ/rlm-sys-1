...
int32_t term(int op, int port, int data) {
    static int tNpipe_fdesc[2][2]; /* rx,tx */
    static char *tNpipe_fname[2][2] = {
        { /*rx*/ "/tmp/oskT0rx|", /*tx*/ "/tmp/oskT0tx|" },
        { /*rx*/ "/tmp/oskT1rx|", /*tx*/ "/tmp/oskT1tx|" },
    };
    ...
    switch(op) {
    case TERMINIT:
        status = mkfifo(tNpipe_fname[port][0], 0666 | O_CREAT | O_RDONLY | O_NONBLOCK);
        if( !((status == 0) || (status == EEXIST))
            || ((tNpipe_fdesc[port][0] = open(tNpipe_fname[port][0], 0644 | O_NONBLOCK)) == -1)
          ) {
            perror("fopen: tpipe_fdesc[rx]");
            ret = -1;
        }
        status = mkfifo(tNpipe_fname[port][1], 0666 | O_CREAT | O_WRONLY | O_NONBLOCK);
        if( !((status == 0) || (status == EEXIST))
            || ((tNpipe_fdesc[port][1] = open(tNpipe_fname[port][1], 0644 | O_NONBLOCK)) == -1)
          ) {
            perror("fopen: tpipe_fdesc[tx]");
            ret = -1;
        }
        break;
    case TERMDEINIT:
        close(tNpipe_fdesc[port][0]); /* rx */
        unlink(tNpipe_fname[port][0]);
        close(tNpipe_fdesc[port][1]); /* tx */
        unlink(tNpipe_fname[port][1]);
        break;
    case TERMREAD:
        {
            uint32_t chan;
            ...
            if( (read_status = read(tNpipe_fdesc[port][0], &inData, 1)) > 0 ) {
                ...
                /* TODO: HACK cygwin fifo workaround - if writer closes
                 * fifo, we see EOF errors even if writer reconnects --
                 * reopening our read handle restores fifo connection */
                /* read() returns 0 when called on a fifo that has been
                 * emptied and the writer has closed its end; it returns
                 * -1 therafter, until/unless we re-open our (reader) end. */
                
                if( read_status == 0 ) {
                    /* EOF from fifo writer - reconnect to fifo */
                    close(tNpipe_fdesc[port][0]);
                    tNpipe_fdesc[port][0] = open(tNpipe_fname[port][0], 0644 | O_NONBLOCK);
                }
                else {
                    /* no data in fifo to read */
                    printf("[no data for TERM %d]\n", port);
                }
            }
        }
        break;
    case TERMWRITE:
        {
            ...
            write(tNpipe_fdesc[port][1], &outData, 1);
