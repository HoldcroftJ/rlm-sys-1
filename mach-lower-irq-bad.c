...
#define CON0_CTLb       0x01 /* CON0_CTL[7:0] MODE1,MOD0,x,x,x,x,PEND,IRQEN */
...
#define TERM_CTLb      0x05    /* Tn_CTL[6:0] RESVD,BR1,BR0,TXE,RXF,RXV,PEND,IRQEN */
#define   TERM_CTLb_BR1    0x40
#define   TERM_CTLb_BR0    0x20
#define   TERM_CTLb_TXE    0x10
#define   TERM_CTLb_RXF    0x08
#define   TERM_CTLb_RXV    0x04
#define   TERM_CTLb_PEND   0x02
#define   TERM_CTLb_IRQEN  0x01
...
int32_t term(int op, int port, int data) {
    static uint32_t term_cheezy_tx_time[2] = {0,0};  /* To emulate (badly) rs-232 tx time */
    static uint32_t term_cheezy_rx_timer = 0u;
    ...
    case TERMINIT:
        {
            term_cheezy_tx_time[0] = 0u;
            term_cheezy_tx_time[1] = 0u;
...
#endif
            if( (read_status = recvfrom(tN_fdesc[port], &inData, 1, 0, (struct sockaddr *)(&clisock), &clisize)) == 1 ) {
                //            if( (read_status = recv(tN_fdesc[port], &inData, 1, 0)) == 1 ) {
                BYTE_AT(GIO,chan+TERM_RXb) = inData;
#ifdef TRACE
                _OK; printf("TERM %d received: %02x '%c'\n", port, inData, (inData < ' ' ? '*' : inData)); N_;
#endif
                if( BYTE_AT(GIO,chan+TERM_CTLb) & TERM_CTLb_RXF ) {
                    /* RX already full, overflow error! */
                    _ERR printf("TERM %d RX ovf\n", port); N_;
                    BYTE_AT(GIO,chan+TERM_CTLb) |= TERM_CTLb_RXV; /* set RXV */
                }
                BYTE_AT(GIO,chan+TERM_CTLb) |= (TERM_CTLb_RXF | TERM_CTLb_PEND);
                if( BYTE_AT(GIO,chan+TERM_CTLb) & TERM_CTLb_IRQEN ) {
    ...
    case TERMWRITE:
        {
            ...
            if( BYTE_AT(GIO,chan+TERM_CTLb) & TERM_CTLb_TXE ) {
                outData = BYTE_AT(GIO,chan+TERM_TXb);
#ifdef TRACE
                printf("calling sendto(), %d '%c'\n", outData, (outData < ' ' ? '*' : outData));
                getchar();
#endif
                sendto(tN_fdesc[port], &outData, 1, 0, (const struct sockaddr *)&clisock, sizeof(clisock));
                //            send(tN_fdesc[port], &outData, 1, 0);
                BYTE_AT(GIO,chan+TERM_CTLb) &= ~TERM_CTLb_TXE; /* TXE=0: busy until...*/
                term_cheezy_tx_time[port] = m68k_get_cycles_counter() + 128;
            }
        }
        break;
    case TERMSVC:
        /** Emulate arrival of data at approximately 31250bps */
        if( (term_cheezy_rx_timer++ % (256/EMU_TIMESLICE)) == 0u ) {
            (void)term(TERMREAD, 0, 0);
            (void)term(TERMREAD, 1, 0);
        }
        /* TODO: Consider sending IRQ on TXE */
        if( term_cheezy_tx_time[0] < m68k_get_cycles_counter() ) {
            BYTE_AT(GIO,TERM0+TERM_CTLb) |= TERM_CTLb_TXE;
        }
        if( term_cheezy_tx_time[1] < m68k_get_cycles_counter() ) {
            BYTE_AT(GIO,TERM1+TERM_CTLb) |= TERM_CTLb_TXE;
        }
        break;
    default:
        ret = -1;
...
int gio_byte_read(int address)
{
...
#if 0 /* N1: Only write to CTLb clears PEND and ACKS IRQ -rlm 2012-08-04 */
        BYTE_AT(GIO,offset) &= ~(0x02); /* PEND=0 - ack IRQ */
        {
            int status = m68k_lower_irq(TIM0_IRQLVL);
#ifdef SHOW_IRQ_ERRORS
            if( status != M68K_OK ) {
                _ERR; printf("[TIM0 lower irq returned %d]\n", status); N_;
            }
            else {
                _OK; printf("[TIM0 lower irq - acked]\n", status); N_;
            }
#endif
#endif /* N1: */                
        break;
    ...
    case TIMT+TIMT_LOAD:
        ...
        {
            int status = M68K_OK; //m68k_lower_irq(TIMT_IRQLVL);
#ifdef SHOW_IRQ_ERRORS
            if( status != M68K_OK ) {
                _ERR; printf("[TIMT lower irq returned %d]\n", status); N_;
            }
            else {
                _OK; printf("[TIMT lower irq - acked]\n", status); N_;
            }
#endif
        }
        break;
    ...
    case TERM1+TERM_RXb:
        ...
        {
            int status = m68k_lower_irq(TERMn_IRQLVL);
#ifdef SHOW_IRQ_ERRORS
            if( status != M68K_OK ) {
                _ERR; printf("[TERM lower irq returned %d]\n", status); N_;
            }
            else {
                _OK; printf("[TERM lower irq - acked]\n", status); N_;
            }
#endif
        }
        break;
    ...
    if( term_cheezy_tx_time <= m68k_get_cycles_counter() ) {
        int chanBase = offset & ~0x0Fu;  /* TERMn BASE */
        BYTE_AT(GIO,chanBase+TERM_CTLb) |= TERM_CTLb_TXE; /* Set TXE - byte sent */
    }
#endif
    ...
    case TERM1+TERM_TXb:
        {
            ...
            BYTE_AT(GIO,chan+TERM_CTLb) &= ~TERM_CTLb_TXE; /* Clear TXE */
        }
        break;
    case TERM0+TERM_CTLb:
    case TERM1+TERM_CTLb:
        {
            ...
            BYTE_AT(GIO,offset) &= ~0x77; /* preseve non-writable bits...*/
...
void show_periph_state(void) {
    printf("T0: RXD:%02x TXD:%02x BR1:%d BR0:%d TXE:%d RXF:%d RXV:%d PEND:%d IRQEN:%d\n",
           BYTE_AT(GIO+TERM0,TERM_RXb),
           BYTE_AT(GIO+TERM0,TERM_TXb),
           (BYTE_AT(GIO+TERM0,TERM_CTLb) & TERM_CTLb_BR1) != 0,
           (BYTE_AT(GIO+TERM0,TERM_CTLb) & TERM_CTLb_BR0) != 0,
           (BYTE_AT(GIO+TERM0,TERM_CTLb) & TERM_CTLb_TXE) != 0,
           (BYTE_AT(GIO+TERM0,TERM_CTLb) & TERM_CTLb_RXF) != 0,
           (BYTE_AT(GIO+TERM0,TERM_CTLb) & TERM_CTLb_RXV) != 0,
           (BYTE_AT(GIO+TERM0,TERM_CTLb) & TERM_CTLb_PEND) != 0,
           (BYTE_AT(GIO+TERM0,TERM_CTLb) & TERM_CTLb_IRQEN) != 0);
    printf("T1: RXD:%02x TXD:%02x BR1:%d BR0:%d TXE:%d RXF:%d RXV:%d PEND:%d IRQEN:%d\n",
           BYTE_AT(GIO+TERM1,TERM_RXb),
           BYTE_AT(GIO+TERM1,TERM_TXb),
           (BYTE_AT(GIO+TERM1,TERM_CTLb) & TERM_CTLb_BR1) != 0,
           (BYTE_AT(GIO+TERM1,TERM_CTLb) & TERM_CTLb_BR0) != 0,
           (BYTE_AT(GIO+TERM1,TERM_CTLb) & TERM_CTLb_TXE) != 0,
           (BYTE_AT(GIO+TERM1,TERM_CTLb) & TERM_CTLb_RXF) != 0,
           (BYTE_AT(GIO+TERM1,TERM_CTLb) & TERM_CTLb_RXV) != 0,
           (BYTE_AT(GIO+TERM1,TERM_CTLb) & TERM_CTLb_PEND) != 0,
           (BYTE_AT(GIO+TERM1,TERM_CTLb) & TERM_CTLb_IRQEN) != 0);
}
    ...
    /* Start emulation */
    {
        ...
        do {
            ...
            if( (cpu_status = m68k_get_cpu_state()) == M68K_OK ) {
                ...
                if( (cpu_status != 16) && (cpu_status != 128) ) {
